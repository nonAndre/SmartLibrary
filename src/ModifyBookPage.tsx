...
export default function ModifyBookPage() {
    ...
    const [relaseYear,setrelaseYear] = useState<string | undefined>(bookData.DataUscita);
    const [casaEditrice,setCasaEditrice] = useState<string | undefined>(bookData.casaEditrice);
    const [category,setCategory] = useState<string[] | undefined>(bookData.Categoria);
  ...
  const modifyBook = async () => {
      ...
      if (!title.trim() ||!relaseYear?.trim() || category?.length == 0 || !casaEditrice?.trim() || authors?.length === 0 ||authors?.some(author => !author.trim()) ) 
      {
     ...
     <div className="flex flex-col items-center w-9/10 h-full pt-6 px-6">
         { isGeminiOpen ? (<GeminiAssistantPanel/>):(<div></div>)
          ...
          <form className="flex flex-col w-full space-y-4">
            ...
            <input type="text"  className="border border-gray-400 rounded-md px-3 pt-2  w-8/10" onChange={getTitle} value={title}/>
            
            <div className="space-y-2 pt-4">
              <h1 className="font-bold">Inserisci l'autore o gli autori del libro</h1>
            ...
            <input type="string"  className="border border-gray-400 rounded-md px-3 py-2  w-8/10" onChange={getYear} value={relaseYear}/>
             <h1 className="font-bold">Inserisci la casa editrice del libro</h1>
            <input type="text" className="border border-gray-400 rounded-md px-3 py-2  w-8/10" onChange={getCasaEd} value={casaEditrice}/>
             <h1 className="font-bold">Inserisci la categoria del libro</h1>
            <input type="text"  className="border border-gray-400 rounded-md px-3 py-2  w-8/10" onChange={getCategory} value={category}/>
           ...
           <div className="flex flex-row justify-between  w-8/10 py-2">
                ...
                <button  type="button" className="flex mt-4 bg-blue-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-blue-600 cursor-pointer" onClick={()=>modifyBook()}>
                  Modifica
                </button>
          </div>
