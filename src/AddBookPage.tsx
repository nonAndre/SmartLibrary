...
import { collection, query, getDocs, where,  updateDoc, setDoc, doc} from "firebase/firestore";
 ...
 const userId = user?.uid;
  ...
  const addBookToArchive = async () => {
                if (!userId) {
                 ...
                 if (!title.trim() ||!relaseYear?.trim() || category?.length == 0 || !casaEditrice?.trim() || authors?.length === 0 ||authors?.some(author => !author.trim()) || !isbn.trim() ) 
                 {
                 ...
                 let geminiResponse = "";
                  try {
                    geminiResponse = await gemini(`quali possono essere dei tag descrittivi ma genereci del libro <<${title}>> per fini di ricerca?La riposta deve essere nel formato:  cat1,cat2... e basta`);
                  } catch (error) {
    ...
    <div className="flex h-screen bg-blue-300 items-center justify-center pt-16 ">
     ...
     <div className="flex flex-col items-center w-9/10 h-full pt-6 px-6">
          ...
          <form className="flex flex-col w-full space-y-4">
            <h1 className="font-bold">Inserisci ISBN per inserimento automatico oppure compila i campi sottostanti</h1>
             <div className="flex flex-row gap-5 items-center">
              <input type="text" placeholder="ISBN Code" className="border border-gray-400 rounded-md px-3 py-2 w-8/10" onChange={getIsbn} value={isbn}/>
              <div className="cursor-pointer" onClick={()=>{fillFields(isbn)}}>
                  <BsClipboardCheck size={24}/>
              </div>
            </div> 
             <h1 className="font-bold pt-4">Inserisci l'autore o gli autori del libro</h1>
            <input type="text" placeholder="Titolo" className="border border-gray-400 rounded-md px-3 pt-2  w-8/10" onChange={getTitle} value={title}/>
            ...
            <div className="space-y-2 pt-4">
              ...
              {authors?.map((author, index) => (
                ...
                />
                <BsTrash3 size={24} color="red" className="cursor-pointer" onClick={()=>removeAuthor(index)}/>
                </div>
              ))}
              <button type="button" onClick={addAuthor} className="text-blue-500 hover:underline">
                + Aggiungi un autore
              </button>
            </div>

            <h1 className="font-bold">Inserisci l'anno di uscita del libro</h1>
            <input type="string" placeholder="Anno di pubblicazione" className="border border-gray-400 rounded-md px-3 py-2  w-8/10" onChange={getYear} value={relaseYear}/>
            <h1 className="font-bold">Inserisci la casa editrice del libro</h1>

            <input type="text" placeholder="Casa Editrice" className="border border-gray-400 rounded-md px-3 py-2  w-8/10" onChange={getCasaEd} value={casaEditrice}/>
            <h1 className="font-bold">Inserisci la categoria del libro</h1>
            <input type="text" placeholder="Categoria" className="border border-gray-400 rounded-md px-3 py-2  w-8/10" onChange={getCategory} value={category}/>
            
           <div className="flex flex-row justify-between  w-8/10 py-2">
                <button type="button" className="flex mt-4 bg-red-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-red-600 cursor-pointer" onClick={()=>{
                  clearFields();
                  navigate('/home');
                }}>
                  Annulla
                </button>
                <button  type="button" className="flex mt-4 bg-blue-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-blue-600 cursor-pointer" onClick={()=>addBookToArchive()}>
                  Aggiungi
                </button>
          </div>
